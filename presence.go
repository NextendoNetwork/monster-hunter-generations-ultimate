// presence.go: every PID sending PRUDP packets is playing MHGU right now; report active PIDs to nextendo-account every 30s.
package main
...
func startPresenceReporter() {
	...
	go func() {
		for {
			...
			body, err := json.Marshal(map[string]any{"appId": mhguAppID, "status": presenceStatus, "pids": active})
			if err != nil {
				continue
			}
			...
			if err != nil {
				fmt.Printf("[MHGU Presence] batch report failed: %v\n", err)
				continue
			}
			if resp.StatusCode != http.StatusOK {
				respBody, _ := io.ReadAll(io.LimitReader(resp.Body, 512))
				fmt.Printf("[MHGU Presence] batch report rejected: HTTP %d: %s\n", resp.StatusCode, respBody)
			}
			resp.Body.Close()
		}
	}()
